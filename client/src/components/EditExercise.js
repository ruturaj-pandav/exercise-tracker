...
export default function EditExercise() {
  ...
  let ex = location.state.ex;

  let navigate = useNavigate();
  useEffect(() => {
    getUsers();
  }, []);
  async function EditExerciseFunction() {
    if (
      username !== "" &&
      date !== "" &&
      duration !== "" &&
      ...
      let response = await axios.post(
        `http://localhost:8000/edit-exercise/${exercise_id}`,
        {
          username,
          description,
          duration,
  ...
  const [username, setusername] = useState(location.state.ex.username);
  const [description, setdescription] = useState(location.state.ex.description);
  const [date, setdate] = useState(location.state.ex.date);
  const [duration, setduration] = useState(location.state.ex.duration);
  ...
  return (
    <>
      ...
      <div className="w-full lg:w-1/2 lg:mx-auto  my-8 border shadow p-5">
        {allusers.length > 0 ? (
          <form
            className="mx-2"
            onSubmit={(e) => {
